...
function removeFirstOccurrences(str, val) {
  if (str.indexOf(val) >= 0) {
    return (
      str.slice(0, str.indexOf(val)) + str.slice(str.indexOf(val) + val.length)
    );
  }
  return str;
}
...
function removeLastOccurrences(str, val) {
  if (str.indexOf(val) >= 0) {
    return (
      str.slice(0, str.lastIndexOf(val)) +
      str.slice(str.lastIndexOf(val) + val.length)
    );
  }
  return str;
}
...
function formatTime(min, sec) {
  const a = `${min}`.padStart(2, 0);
  const b = `${sec}`.padStart(2, 0);
  return `${a}:${b}`;
}
...
function isPalindrome(str) {
  const formatted = str.toLowerCase().replace(/\W/g, '').split('').join('');
